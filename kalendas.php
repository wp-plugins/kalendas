...
function kalendas_list_events($source, $rand) {
	...
	return apply_filters('the_content', $answer).$script.$link;
		...
		if($data[0]=="<") {
			...
			foreach($data->entry as $item) {
				...
				for($aux = 0; $aux<count($when_attr); $aux++) {
					$attr = $gd->eventStatus->attributes();
					if(strstr($attr->value,'confirmed')) {
						$when_attr_aux = $when_attr[$aux]->attributes();
						$out.="<event><title>".htmlspecialchars($item->title)."</title><where>{$where_attr->valueString}</where><start>{$when_attr_aux->startTime}</start><end>{$when_attr_aux->endTime}</end><description>".htmlspecialchars($item->content)."</description></event>"; //Add the entry to the XML file
					}
				}
			}
			$out.="</events>";
